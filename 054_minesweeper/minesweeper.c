...
board_t * makeBoard(int w, int h, int numMines) {
  board_t * zboard = malloc(sizeof(*zboard));
  zboard->height = h;
  zboard->width = w;
  zboard->totalMines = numMines;
  zboard->board = malloc(h * sizeof(*zboard->board));
  for (int i = 0; i < h; i++) {
    zboard->board[i] = malloc(w * sizeof(*zboard->board[i]));
    for (int j = 0; j < w; j++) {
      zboard->board[i][j] = UNKNOWN;
    }
  }
  while (numMines > 0) {
    addRandomMine(zboard);
    numMines--;
  }
  //WRITE ME!
  return zboard;
...
int countMines(board_t * b, int x, int y) {
  int n = 0;
  int temp = 0;
  if ((temp = in(y - 1, x - 1, b)) == 1 && IS_MINE(b->board[y - 1][x - 1])) {
    n++;
  }
  if ((temp = in(y - 1, x, b)) == 1 && IS_MINE(b->board[y - 1][x])) {
    n++;
  }
  if ((temp = in(y - 1, x + 1, b)) == 1 && IS_MINE(b->board[y - 1][x + 1])) {
    n++;
  }
  if ((temp = in(y, x - 1, b)) == 1 && IS_MINE(b->board[y][x - 1])) {
    n++;
  }
  if ((temp = in(y, x + 1, b)) == 1 && IS_MINE(b->board[y][x + 1])) {
    n++;
